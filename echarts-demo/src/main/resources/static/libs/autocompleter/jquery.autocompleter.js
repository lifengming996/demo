...
;(function ($, window) {
     ...
     * @param hint [boolean] <false> "Add hint to input with first matched label, correct style should be installed"
        ...
        if (!$node.hasClass('autocompleter-node')) {
            ...
            if (typeof opts.source === 'string' && (opts.source.slice(-5) === '.json' || opts.asLocal === true)) {
                $.ajax({
                    ...
                    opts.source = opts.formatResult(response) || response;
                });
            }
    ...
    function _response(response, data) {
        response = data.formatResult(response) || response;
